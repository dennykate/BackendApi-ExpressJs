...
const db = mongojs("mrpussy", ["categories"]);
...
router.get("/", (req, res) => {
  ...
  db.categories
    ...
    .limit(limit, (err, data) => {
      ...
      db.categories.find((err, totalData) => {
...
router.get("/:slug", (req, res) => {
  ...
  db.categories.count({ slug: slug }, (err, count) => {
    if (err) return res.sendStatus(500);

    if (count) {
      db.categories.find({ slug: slug }, (err, data) => {
        if (err) return res.sendStatus(500);
...
router.post(
  ...
  (req, res) => {
    ...
    db.categories.insert(record, (err, data) => {
      if (err) return res.sendStatus(500);
